...
class FlowmatchingUnetLowdimPolicy(BaseLowdimPolicy):
    ...
    def conditional_sample(
        ...
        global_cond=None,
        generator=None,
        # keyword arguments to flow_matcher.step
        **kwargs,
    ):
        model = self.model
        ...
        # set step values
        flow_matcher.set_timesteps(self.num_inference_steps)

        for t in flow_matcher.timesteps:
    ...
    def compute_loss(self, batch):
        ...
        # apply conditioning
        xt[condition_mask] = trajectory[condition_mask]

        # Predict the noise residual
        pred = self.model(xt, timesteps, local_cond=local_cond, global_cond=global_cond)

        pred_type = self.flow_matcher.config.prediction_type
