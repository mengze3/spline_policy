...
class DiffusionUnetImagePolicy(SplinePolicyMixin, BaseImagePolicy):
    def __init__(
        self,
        shape_meta: dict,
    ...
    def predict_action(
        ...
        # normalize input
        nobs = self.normalizer.normalize(obs_dict)  # agent_pos, image
        value = next(iter(nobs.values()))
        B, To = value.shape[:2]
        T = self.poly.nbSeg + 2
        Da = self.action_dim
        Do = self.obs_feature_dim  # obs_encoder.output_shape()[0]
        To = self.n_obs_steps
        ...
        if self.obs_as_global_cond:
            ...
            global_cond = nobs_features.reshape(
                B, -1
            )  # (B, n_obs_steps * obs_feature_dim)
        ...
        nwparams_pred = nwparams[..., :Da]
        wparams = self.normalizer["action"].unnormalize(nwparams_pred)

        # sample trajectory
        sample_num = self.n_action_steps + To - 1
    ...
    def compute_loss(self, batch):
        ...
        trajectory, sample_trajectory = self._build_spline_target(nactions, horizon)
        ...
        timesteps = torch.randint(
            ...
            (bsz,),
            device=sample_trajectory.device,
        ).long()
        ...
        # apply conditioning
        noisy_trajectory[condition_mask] = sample_trajectory[condition_mask]
