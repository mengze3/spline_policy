...
class TrainDiffusionUnetLowdimWorkspace(BaseWorkspace):
    ...
    def run(self):
        ...
        print(f"env_runner type: {type(env_runner)}, env_runner: {env_runner}")
            ...
            for local_epoch_idx in range(cfg.training.num_epochs):
                ...
                if (self.epoch % cfg.training.sample_every) == 0:
                    with torch.no_grad():
                        ...
                        # sample gt_action to make it compatible with pred_action
                        B, T, D = pred_action.shape
                        gt_action = torch.nn.functional.interpolate(
                            gt_action.permute(0, 2, 1),
                            size=T,
                            mode="linear",
                            align_corners=True,
                        ).permute(0, 2, 1)
